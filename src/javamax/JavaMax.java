...
public class JavaMax extends JFrame {
	...
	private JButton scoreButton = new JButton("점수보드");// 점수보드 버튼
	...
	public JavaMax() {
		...
		// Track으로 변수 초기화 순서적으로 값이 들어간다 (제목,게임 선택 창표지,실행했을때 표지, 하이라이트, 해당곡,곡제목) 여기는 DB로
		// 업이트 할 예정
		...
		scoreButton.setVisible(false); // 초기에는 숨김
	...
	public void screenDraw(Graphics2D g) {// backgroud이미지를screenImage넣는다
		...
		if (isMainScreen) {// true경우 출력
			g.drawImage(selectedImage, 340, 100, null);
			g.drawImage(titleImage, 340, 70, null);

			// 최고 점수 표시
			if (trackList.size() > nowSelected) {
				String trackName = trackList.get(nowSelected).getTitleName();
				int bestScore = dbManager.getBestScore(trackName);

				// 최고 점수 텍스트 스타일 설정
				g.setColor(Color.WHITE);
				g.setFont(getKoreanFont(java.awt.Font.BOLD, 20));

				// 최고 점수 표시
				String bestScoreText = "최고 점수: " + bestScore;
				g.drawString(bestScoreText, 340, 450);
			}
		}
		if (isGameScreen) {
	...
	public void gameStart(int nowSelected, String difficulty) {// 모드의 정보를 받는다
		...
		game = new Game(trackList.get(nowSelected).getTitleName(), difficulty,
				trackList.get(nowSelected).getGameMusic());// 현재선태된 곡을
		game.start();// 런함수 실행 (노트 생성)
		setFocusable(true);// 키보드 포커스를 맞춤
	}

	public void enterMain() {
		// Music selectedMusic = new Music("Wellerman.mp3",true);//넘어 올떄 선택할 곡 재생(하이라이트
		// 곡재생 30초)
		...
		scoreButton.setVisible(true);// 점수보드 버튼
